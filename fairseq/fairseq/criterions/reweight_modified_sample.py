...
def mml_label_smoothed_nll_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample,
    ...
    nos_loss = 0
    loss_list = []
    if "test" in tmp_sample.keys():
        for candi in range(n_list[0]):
            nos_loss = nos_loss + tmp_loss_list[candi]
...
@register_criterion('reweight_modified_sample')
class ReweightModifiedSample(FairseqCriterion):
