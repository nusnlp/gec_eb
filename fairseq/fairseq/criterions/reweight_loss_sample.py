...
def max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, ori_sample, sample_size, epsilon,
    ...
    if nos_prob is not None:
        a, b = ori_sample["target"].shape
        sample = torch.multinomial(nos_prob, a, replacement=True)
    for i in range(n_list[0]):
        ...
        if nos_prob is not None:
            tmp_mask = sample.eq(i)
            new_mask = tmp_mask.unsqueeze(-1).expand_as(new_nll_other_matrix)
            if not new_mask.all():
                new_nll_other_matrix.masked_fill_(~new_mask, 0.)
...
@register_criterion('reweight_sample')
class ReweightSample(FairseqCriterion):
