...
def max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, ori_sample, sample_size, epsilon,
                    ignore_index=None, nos_prob=None, sub=None, key=None):
    '''
    ...
    #         weighted_loss = final_new_nll_other_matrix[candi] * nos_prob[candi]
    #         local_loss = weighted_loss.sum()
    #         final_loss = final_loss + local_loss
    #     # print(nos_prob)
    #     # print("Calculating the weighted loss and obtain the sum")
    ...
    #     # final_new_nll_other_matrix = []
    #     tmp_sample = copy.deepcopy(ori_sample)
    #     tmp_sample['net_input']['prev_output_tokens'] = ori_sample['net_input']['prev_output_tokens_neg_padded'][sub]
    #     # tmp_sample['target'] = ori_sample['neg_target'][i]
    #     neg_sample = copy.deepcopy(tmp_sample)
    #     del neg_sample['net_input']['prev_output_tokens_neg_padded']
    #     neg_net_output = model(**neg_sample['net_input'])
    ...
    #     # final_new_nll_other_matrix.append(new_nll_other_matrix)
    #     final_loss = 0
    #     # weighted_loss = final_new_nll_other_matrix[0]
    #     local_loss = new_nll_other_matrix.sum()
    #     final_loss = final_loss + local_loss
    #     # print("Calculating the loss for the {}th candidate".format(sub))
    # else:
    import copy
    a, b = pos_ori_target.shape
    target = pos_ori_target.view(a, b, 1)
    ...
    if nos_prob is not None:
        for candi in range(n_list[0]):
            ...
            local_loss = weighted_loss.sum() * nos_prob[candi]
            final_loss = final_loss + local_loss
    else:
        for candi in range(n_list[0]):
            weighted_loss = final_new_nll_other_matrix[candi]
            local_loss_list.append(weighted_loss.sum())
            local_loss = weighted_loss.sum() / n_list[0]
            final_loss = final_loss + local_loss
        # print("Calculating the loss for the sum of all candidates")
    return final_loss, local_loss_list
...
def mml_label_smoothed_nll_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample,
    ...
    mml, loss_list = max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample, sample_size, epsilon,
                          ignore_index, nos_prob)
    # print("Inside the reweight_all_multiple function, the ori_prob is : {}".format(ori_prob))
    if ori_prob is not None:
        # print("Returing weighted loss with {}, {}".format(ori_prob[0], ori_prob[1]))
        # print("Loss value: ce_loss: {}, mml: {}".format(ce_loss, mml))
        loss = ori_prob[0] * ce_loss + ori_prob[1] * mml
    else:
        # print("Returning the equal weight loss")
        loss = 0.5 * ce_loss + 0.5 * mml
        loss_list.append(ce_loss)
        loss_list.append(mml)
    return loss, loss_list, nll_loss

@register_criterion('reweight_all_multiple')
class ReweightALLMultiple(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, ori_prob=None, nos_prob=None, update_theta=False, sub=None, keys=None):
        ...
        if 'valid' in sample.keys():
            ...
            sample_size = sample['target'].size(0) if self.args.sentence_avg else sample['ntokens']
            loss, loss_list, nll_loss = self.mml_compute_loss(model, n_list, pos_net_output, tmp_sample, pos_sample, sample_size,
            ...
            if ori_prob is not None:
                # print("Returning loss: {}".format(loss))
                return loss, sample_size, logging_output
    ...
    def mml_compute_loss(self, model, n_list, pos_net_output, tmp_sample, pos_sample, sample_size, reduce=True,
        ...
        loss, loss_list, nll_loss = mml_label_smoothed_nll_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample,
                                                     sample_size, self.eps,
