...
def mml_label_smoothed_nll_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample,
    ...
    if ori_prob is None:
        if keys == "original":
            ...
            # print("Returning the not weighted loss, including two loss component")
            # print("==========================")
            # print("original loss: {}, noisy loss: {}, ori_weight: {}".format(ce_loss, mml, ori_weight))
            return loss, nll_loss
    else:  # sub=None
        mml = max_margin_loss(model, pos_ori_lprobs, pos_ori_target, n_list, tmp_sample, sample_size, epsilon,
                              ignore_index, nos_prob=nos_prob)
        loss = ori_prob[0] * ce_loss + ori_prob[1] * mml
        # print("Returning the weighted loss, including two weights")
        # print("==========================")
        return loss, nll_loss

@register_criterion('reweight_all')
class ReweightALL(FairseqCriterion):
