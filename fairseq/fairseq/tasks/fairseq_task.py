...
class FairseqTask(object):
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False, ori_prob=None, nos_prob=None,
                   update_theta=False, loss_copy=None, data_actor_out=None, backward=True,
                   sentence_level=False, sub=None, element=None, keys=None):
        """
        ...
        """
        model.train()
        loss, sample_size, logging_output = criterion(model, sample, ori_prob=ori_prob, nos_prob=nos_prob,
                                                      update_theta=update_theta, sub=sub, keys=keys)
        if ignore_grad:
            loss *= 0
        if backward:
    ...
    def hu_valid_step(self, sample, model, criterion):
        model.train()
        loss, sample_size, logging_output = criterion(model, sample)
        return loss
