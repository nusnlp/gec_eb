...
class Adam(torch.optim.Optimizer):
    ...
    def step(self, closure=None):
        ...
        if closure is not None:
            loss = closure()
        # print("Entering the step function")
        for group in self.param_groups:
            for p in group['params']:
                ...
                # if len(state) == 0:
                #     initial = True
                # elif len(state) == 1 and 'param_copy' in state.keys():
                #     initial = True
                # else:
                #     initial = False
                # print(state.keys())
                if len(state) == 0 or (len(state) == 1 and 'dev_grad' in state.keys()):
                    state['step'] = 0
